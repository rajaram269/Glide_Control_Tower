...
_SYSTEM = (
    ...
    "  • FIRST check what columns actually EXIST before declaring a metric non-computable. "
    "Net sales is COMPUTABLE, and you MUST list it in computable_metrics, whenever ANY of "
    "these hold — do not require signed negatives if the breakup is present as columns:\n"
    "      (a) an explicit net column AND a gross/MRP column both exist (net is present "
    "directly; discount = gross − net is derivable) — e.g. Net_Sales alongside "
    "MRP_Sales/MRP; OR\n"
    "      (b) an explicit discount/markdown/BFD/rebate column exists alongside gross "
    "(net = gross − discount); OR\n"
    "      (c) returns/refunds are represented as signed negative rows (a returns/refund "
    "transaction type AND negatives>0), so returns net out.\n"
    "    When net is computable, say HOW in computable_metrics, naming the exact columns "
    "(e.g. 'net sales: Net_Sales column present; discount = MRP_Sales − Net_Sales'). Do "
    "NOT put net in not_computable in this case.\n"
    "  • ONLY conclude net is NOT derivable when NONE of (a)/(b)/(c) hold — i.e. the sole "
    "measure is a gross/MRP amount, there is no net column, no discount column, and "
    "negatives≈0. A column literally named 'net' with negatives≈0 AND no gross/discount "
    "counterpart to corroborate it is suspect (possibly unadjusted gross mislabeled) — "
    "note that in caveats — but the PRESENCE of both a gross and a net column is strong "
    "evidence the net figure is real and usable; prefer that over suspicion.\n"
